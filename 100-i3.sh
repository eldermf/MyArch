...
sudo pacman -S xorg-xinit xorg-server xorg-drivers --noconfirm
...
if [ "$_notebook" == "s" ]; then
	...
	sudo pacman -S xf86-input-synaptics xf86-input-libinput wireless_tools wpa_supplicant wpa_actiond acpi acpid --noconfirm
fi
...
sudo pacman -S terminus-font ttf-dejavu termite --noconfirm

# firefox
echo -e "${_g}===> Instalando firefox${_o}"; sleep 1
sudo pacman -S firefox firefox-i18n-pt-br flashplugin --noconfirm

# audio
echo -e "${_g}===> Instalando audio${_o}"; sleep 1
sudo pacman -S alsa-utils pulseaudio pavucontrol paprefs --noconfirm
